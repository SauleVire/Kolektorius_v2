...
void menuChangeEvent(MenuChangeEvent changed)  // funkcja klasy MenuBackend 
{
  ...
  */
  ...
  lcd.setCursor(0,0); 
  if(c==1)                                                // je�li to menu g�owne (shortkey=1) to:
    {InMenu =true;
    lcd.write(3);                                         // strza�ka w lewo
    strcpy(eilute1,changed.to.getName());                  // tworzymy napis w pierwszej linii
    lcd.print(eilute1);                                    // wy�wietlamy j�
    lcd.setCursor(15,0);lcd.write(4);                     // strza�ka w prawo
    lcd.setCursor(0,1);lcd.write(5); lcd.print("                  ");   
    lcd.setCursor(15,1);lcd.write(5);                     // strza�ka w d��
    lcd.setCursor(0,2);lcd.print("                    "); // i�valoma 3 eilut�, kai atsiduriama meniu nr.1
    }
    if(c==2)                                              // je�li to podmenu dla dziecka - (shortkey=2) to:
    {InMenu =true;
    lcd.print("*");                                       // rysujemy gwiazdk�
    strcpy(eilute2,changed.to.getName());                  // tworzymy napis w pierwszej linii
    lcd.print(eilute1);                                    // wy�wietlamy j�
    lcd.setCursor(15,0);lcd.print("*");                   // gwiazdka 
    lcd.setCursor(0,1);lcd.write(6); lcd.print("                  ");                     // druga eilute i strza�ka powrotu (arrowBack)
    lcd.setCursor(1,1);lcd.print(changed.to.getName());                      // wy�wietlamy nazwe "dziecka"
    lcd.setCursor(15,1);lcd.write(7);                     // strza�ka g�ra-d��
    }
    
    if(c==3)                                              // je�li dziecko  ma dziecko - (shortkey =3) to:
    {InMenu =true;
    lcd.print("*");                                       // gwiazdka
    strcpy(eilute2,changed.to.getName());                  // kopiujemy akt. nazwe opcji menu do klaviaturos_pasikeitimasiennej eilute2
    lcd.print(eilute1);                                    // i wy�wietlamy pierwsz� lini�
    lcd.setCursor(15,0);lcd.print("*");                   // gwiazdka
    lcd.setCursor(0,1);lcd.write(6);                      // druga eilute i strza�ka arrowBack
    lcd.print(changed.to.getName());                      // wy�wietlamy wnuka w drugiej linii
    lcd.setCursor(15,1);lcd.write(4);                     // strza�ka w prawo bo s� wnuki
    }
    
    if(c==4)                                              // je�li to wnuk  (shortkey =4) to:
    {InMenu =true;
    lcd.print("*");                                       // gwaizdka
    lcd.print(eilute2);                                    // w pierwszej linii wy�wietlamy dziecko ( czyli rodzica wnuka) 
    lcd.setCursor(15,0);lcd.print("*");                   // gwaizdka
    lcd.setCursor(0,1);lcd.write(6);                      // druga eilute i strza�ka arrowBack
    lcd.print(changed.to.getName());                      // wy�wietlamy wnuka
    lcd.setCursor(15,1);lcd.write(7);                     // strza�ka g�ra-d�� 
    } 
}
