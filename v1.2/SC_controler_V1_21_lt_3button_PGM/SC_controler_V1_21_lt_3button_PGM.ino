...
//char itemKOLEKTORIUS[] PROGMEM  = "KOLEKTORIUS";
//char itemSkirtumasOn[] PROGMEM  = "Skirtumas on  ";
//char itemSkirtumasOff[] PROGMEM    = "Skirtumas off ";
//char itemSiurblioIj[] PROGMEM  = "Siurblio ij   ";
//char itemExit[] PROGMEM    = "Exit          ";
...
/* __________________________Settings brithness __________________ */
  ...
  lcd.print("                  ");lcd.setCursor(1,1);lcd.print(F("Sviesumas")); // keiciamos reikšmes pavadinimas
...
if (used.item.getName() == "Skirtumas on  ") 
 {       
        ...
        lcd.setCursor(1,1);lcd.print(F("Siurblys")); 
        if (Manual_pump_status == true) lcd.print(F(" -on ")); // 
        if (Manual_pump_status == false) lcd.print(F(" -off")); //
        ...
        while(action!=4)                   // 
         {
           ...
           if(Keyboard_change!=action)           
             {
if(action==0) {Manual_pump_status = false; lcd.setCursor(11,1);lcd.print("off");delay(200);}
if(action==3) {Manual_pump_status = true;  lcd.setCursor(11,1);lcd.print("on ");delay(200);}
             if(action==4) // 0
               {
...
/*  buttonPressed=Read_keyboard(Key_Pin);delay(30);             // read the state of the keyboard:
  if(Keyboard_change!=buttonPressed)                               // if there was a change in the state are:
    {
      switch(buttonPressed)                           // check what was pressed
      {
        ...
        while(action!=4)                   // 
         {
           ...
           if(Keyboard_change!=action)           
             {
             if (action==0) { Converted_Value++; if( Converted_Value>Max_Value)  Converted_Value=Max_Value; lcd.setCursor(11,1);
                                                    if( Converted_Value<10) lcd.print(" ");
                                                      lcd.print( Converted_Value); delay(200);}
             if(action==3)  { Converted_Value--; if( Converted_Value<Min_Value)  Converted_Value=Min_Value; lcd.setCursor(11,1);
                                                     if( Converted_Value<10) lcd.print(" ");
                                                       lcd.print( Converted_Value); delay(200);}
             if(action==4) // 0
               {
...
void navMenu(){
  ...
  case 0: 
        if (menu.getCurrent().getRight() != 0){  menu.moveUp();
                  Serial.print(menu.getCurrent().getName()); Serial.println(" has SUMmenu right");}
    else{  //otherwise, menu has no child and has been pressed. enter the current menu
