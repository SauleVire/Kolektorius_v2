...
#define BackLight_DDR DDRB //#define BackLight_Pin 8 //LCD backlight pin (standart LCD KeeyPad use pin 10)
#define BackLight_DDR_Pin DDB0
#define BackLight_Port PORTB
#define BackLight_Port_Pin PORTB0
...
#define BackLight_DDR DDRB //#define BackLight_Pin 8 //LCD backlight pin (standart LCD KeeyPad use pin 10)
#define BackLight_DDR_Pin DDB1
#define BackLight_Port PORTB
#define BackLight_Port_Pin PORTB1
...
#define Relay_Collector_OFF PORTC |= _BV(PORTC1)
#define Relay_Collector_ON PORTC &= ~_BV(PORTC1)
#define Relay_Thermostat_OFF PORTC |= _BV(PORTC2)
#define Relay_Thermostat_ON PORTC &= ~_BV(PORTC2)
...
{
int stan_Analog =   adc_read(7);delay(30);
...
BackLight_DDR |=_BV(BackLight_DDR_Pin);
/* set pin A1, A0 for output*/
DDRC |= _BV(DDC1);
DDRC |= _BV(DDC2);
...
if (Manual_pump_status == true)  Relay_Collector_ON; //{digitalWrite(Relay_Collector,LOW);}
else{
   if (K-B>=Pump_power_on_difference) Relay_Collector_ON; //digitalWrite(Relay_Collector,LOW);
   if (K-B<=Pump_power_off_difference) Relay_Collector_OFF; //digitalWrite(Relay_Collector,HIGH);
    }
  if (Thermostat_status == 1) 
   {// If the heating mode (Thermostat_status = 1)
    if (T <= temperature_1) Relay_Thermostat_ON; //digitalWrite(Relay_Thermostat,LOW);  
    if (T >= temperature_2) Relay_Thermostat_OFF; //digitalWrite(Relay_Thermostat,HIGH);
   }
   if (Thermostat_status == 2) 
    ...
    if (Thermostat_status == 3) 
     // If you do not need a second relay-mode off
      Relay_Thermostat_OFF; //digitalWrite(Relay_Thermostat,HIGH);
 }
}// === END ===========================================================
////////////////////////////////////////////////////////////////////////
...
ADCSRA |= ((1<<ADPS2)|(1<<ADPS1)|(1<<ADPS0)); //    ADCSRA = (1<<ADEN)|(1<<ADPS2)|(1<<ADPS1)|(1<<ADPS0);
}
