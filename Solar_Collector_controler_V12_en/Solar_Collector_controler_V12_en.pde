...
MenuBackend menu = MenuBackend(menuUseEvent,menuChangeEvent); // menu design 
   //                        ("                ")
   MenuItem P1 =  MenuItem("COLLEKTOR     ",1);
      MenuItem P11 = MenuItem("Difference on ",2);
      MenuItem P12 = MenuItem("Difference off",2);
      MenuItem P13 = MenuItem("Manual pump   ",2);

   MenuItem P2 = MenuItem("TERMOSTAT     ",1);
      MenuItem P21 = MenuItem("temperature 1 ",2);
      MenuItem P22 = MenuItem("temperature 2 ",2);
      MenuItem P23 = MenuItem("Status        ",2);

   MenuItem P3 = MenuItem("SETTINGS      ",1);
      MenuItem P31 = MenuItem("Save          ",2);
      MenuItem P32 = MenuItem("Set default   ",2);
      MenuItem P33 = MenuItem("LCD brightness",2);
...
/* __________________________Settings brithness __________________ */
  ...
  lcd.print("                  ");lcd.setCursor(1,1);lcd.print("brightness"); // keiciamos reikšmes pavadinimas
  lcd.setCursor(12,1);lcd.print(LCD_brightness);lcd.print("0% ");lcd.write(7);                        // dabartine reikšme
  ...
  while(action!=4)                   // Šis ciklas bus kartojamas, kol paspausite mygtuka OK
         {
           ...
           if(Keyboard_change!=action)                    // ruszamy do pracy tylko wtedy gdy Keyboard_changeienil sie stan klawiatury
             {lcd.setCursor(12,1);
             if (action==1) {LCD_brightness++; analogWrite(BackLight_Pin,LCD_brightness*25);delay(300);}
             ...
             if(action==2)  {LCD_brightness--;analogWrite(BackLight_Pin,LCD_brightness*25);delay(300);}
...
if (LCD_brightness < 1)  LCD_brightness = 10;
            ...
            lcd.print("0% ");
             ...
             if(action==4) // jesli wcisnieto OK 
               {
                 lcd.setCursor(0,0);lcd.print(">Brightness OK");delay(2000); // pokazujemy OK przez 2 sek.
...
// Save to EEPROM
     if (used.item.getName() == "Save          ")   // exactly the same string "Save          "
      {
                 SaveConfig();
                 lcd.setCursor(0,0);lcd.print(">Saved OK     ");delay(2000); // show OK for 2 sec
...
// Save to EEPROM
     if (used.item.getName() == "Set default   ")   // exactly the same string "Save          "
      {
                 lcd.setCursor(0,0);lcd.print(">Default OK   ");delay(2000); // show OK for 2 sec
                 lcd.setCursor(0,0);lcd.print("              "); // clear line
                 lcd.setCursor(0,0);lcd.print("*");lcd.print(LCD_string_1);           // reconstruct the previous state at LCD
                 lcd.setCursor(15,0);lcd.print("*");
...
if (used.item.getName() == "Manual pump   ") 
 {       
        ...
        lcd.setCursor(1,1);lcd.print("Manual pump"); 
        if (Manual_pump_status == true) lcd.print("-on"); // 
        if (Manual_pump_status == false) lcd.print("-of"); //
        ...
        while(action!=4)                   // 
         {
           ...
           if(Keyboard_change!=action)           
             {
if(action==1) {Manual_pump_status = false; lcd.setCursor(8,1);lcd.print("off    ");delay(200);}
if(action==2) {Manual_pump_status = true;  lcd.setCursor(8,1);lcd.print("on     ");delay(200);}
             if(action==4) // 0
               {
                 lcd.setCursor(0,0); lcd.print(">Manual pump OK"); delay(2000); // 0
        ...
        while(action!=4)                   // 
         {
           ...
           if(Keyboard_change!=action)           
             ...
             if(action==4) // 0
               {
                 lcd.setCursor(0,0); lcd.print(">Status      OK"); delay(2000); // 0
                 lcd.setCursor(0,0); lcd.print("              "); // 0
                 lcd.setCursor(1,0);lcd.print(LCD_string_1);           // 0
                 menu.moveDown();
               }
...
void LCD_T_template(){
  lcd.setCursor(0,0); lcd.print("C"); lcd.setCursor(6,0); lcd.print(" D");
...
lcd.setCursor(1,0); lcd.print(K); if (K-B>0) {lcd.setCursor(8,0); lcd.print("+");lcd.print((K-B),1);}
                                  ...
                                  if (K-B>=Pump_power_on_difference)  {lcd.setCursor(14,0);lcd.print("C");lcd.write(1);}  
                                  if (K-B<=Pump_power_off_difference) {lcd.setCursor(14,0);lcd.print("C");lcd.write(5);}  
